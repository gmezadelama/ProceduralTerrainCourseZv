...
public class TreeSpawner : MonoBehaviour
{
            ...
            for(int z = 0; z < size; z++)
            {
                if (dataMap[x, z].biome.spawnPrefabs)
                {
                    ...
                    if (density > 1.0f)
                    {
                        ...
                        int extraTreeSpawn = Mathf.RoundToInt(Random.Range(1.0f, density));

                        for (int e = 0; e < extraTreeSpawn; e++) {
                            SpawnTree(dataMap[x, z].position, dataMap[x, z].biome.spawnablePrefabs);
                        }
                    } else
                    {
    ...
    void SpawnTree (Vector3 position, GameObject[] availablePrefabs)
    {
        Vector3 truePos = new Vector3(
            position.x + (Random.value * maxTreeOffset),
            position.y,
            position.z + (Random.value * maxTreeOffset)
        );

        GameObject treeObj = Instantiate(
            availablePrefabs[Random.Range(0, availablePrefabs.Length)],
            truePos,
            Quaternion.identity
        );
